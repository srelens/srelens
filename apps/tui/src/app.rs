            ...
            ActiveView::TuiConfig(cfg_state) => {
                if cfg_state.is_editing {
                    match key.code {
                        ...
                        KeyCode::Enter => {
                            cfg_state.finish_editing(&mut self.tui_config);
                            self.set_toast("Saved ArgoCD Hub setting".to_string(), Theme::status_ok());
                        }
                        KeyCode::Backspace => {
                ...
                match key.code {
                    ...
                    KeyCode::Char('c') | KeyCode::Char('C') => {
                        if cfg_state.selected_field == 4 || cfg_state.selected_field == 5 {
                            cfg_state.clear_current(&mut self.tui_config);
                            self.set_toast("Cleared ArgoCD Hub setting".to_string(), Theme::status_ok());
                        }
                    }
