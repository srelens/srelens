...
impl YamlViewState {
    pub fn new(
        ...
        let lines = yaml_content
            .lines()
            .map(super::sanitize_span_text)
            .collect();
        Self {
            resource_name: name,
            resource_kind: kind,
    ...
    /// Revert the YAML content to the original cluster version before edit.
    pub fn revert_content(&mut self) {
        self.yaml_content = self.original_yaml.clone();
        self.lines = self
            .yaml_content
            .lines()
            .map(super::sanitize_span_text)
    ...
    /// Commit new YAML content as the new baseline after a successful apply.
    pub fn commit_content(&mut self, new_yaml: String) {
        self.original_yaml = new_yaml.clone();
        self.update_content(new_yaml);
    }
    ...
    pub fn spawn_editor(&self) -> Result<Option<String>, String> {
        ...
        let parts: Vec<String> = shlex::split(&editor)
            .unwrap_or_else(|| editor.split_whitespace().map(String::from).collect());
        ...
        let new_content = std::fs::read_to_string(temp_file.path()).map_err(|e| e.to_string())?;
        ...
        if has_match {
            ...
            if trimmed.starts_with('#') {
                ...
                if !val.is_empty() {
                    ...
                    let val_style = if trimmed_val == "true"
                        || trimmed_val == "false"
                        || trimmed_val.parse::<i64>().is_ok()
                    {
                        Style::default().fg(Theme::YELLOW)
