...
import { ExtensionWorkspace, ExtensionLogo, useExtensions, ExtensionResourceNavigation, ExtensionResourceDetails } from "@srelens/ui-next/extensions";
...
export function ClassicAppPage({context,id,page,namespace="",resourceName,onOpenResource,onPage}:{context:string;id:string;page:string;namespace?:string;resourceName?:string;onOpenResource?(name:string,namespace:string):void;onPage(page:string,namespace?:string):void}) {
  ...
  return <ExtensionControlsProvider value={controls}><div className="flex min-h-0 flex-1 flex-col overflow-auto">
    {inventory.status==="loading"?<p className="p-3">Loading app…</p>:inventory.status==="error"?<div role="alert" className="p-3">{inventory.error}<Button onClick={inventory.reload}>Retry</Button></div>:plugin&&contribution?<ExtensionResourceNavigation.Provider value={onOpenResource ? resource=>onOpenResource(resource.name,resource.namespace):undefined}>{resourceName ? <ExtensionResourceDetails fullPage selection={{id,revision:plugin.revision,capability:contribution.capability,context,namespace,name:resourceName}} onChanged={()=>{}}/> : <ExtensionWorkspace key={`${context}/${id}/${plugin.revision}`} context={context} plugin={plugin} page={contribution} namespace={namespace} onPage={onPage}/>}</ExtensionResourceNavigation.Provider>:<p className="p-3">This app page is unavailable. Manage it in Settings → Apps.</p>}
