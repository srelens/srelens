...
1. Call `k8s.getObject` with `context: {{context}}`, `kind: Pod`,
   ...
   find the container with the highest `restartCount` — call it the crashing
   container; in a multi-container pod it is not necessarily the first one
   listed — and read its `lastState.terminated` block: `exitCode`, `reason`,
   `message`. Exit code 137 with reason `OOMKilled` means the container hit
   its memory limit. Exit code 1 or 2 usually means the process itself failed
   and the logs will say why. A `CrashLoopBackOff` waiting reason only tells
   you Kubernetes is backing off; the terminated block says what actually
   happened.
