...
/// Defines `MappedCapability` — everything a resource read can invoke — from
/// ONE list: the enum variants, the `ALL` array the guard tests iterate, and
/// the `capability_id` mapping are all generated from the same entries, so a
/// variant cannot exist without being in `ALL` and having an id decision.
/// That is what actually closes #196's drift class: the old hand-maintained
/// id array could diverge from `plan_read` with the suite green (its
/// exhaustive-`match` guard only forced a no-op arm, demonstrated end to end
/// during #194's review), and even a hand-written enum-plus-`ALL` pair would
/// reopen the same hole one forgotten array entry at a time. Here there is
/// nothing to forget — one entry IS the variant, its membership, and its id.
macro_rules! mapped_capabilities {
    ($($(#[$doc:meta])* $variant:ident => $capability_id:expr),+ $(,)?) => {
        /// Everything a resource read can invoke — a closed enum rather than
        /// bare `&'static str` ids, so the set `plan_read` can name is
        /// compiler-known by construction (#196). Generated by
        /// `mapped_capabilities!`; see the macro's doc for why.
        #[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
        pub enum MappedCapability {
            $($(#[$doc])* $variant,)+
        }

        impl MappedCapability {
            /// Every variant, generated from the same list as the enum
            /// itself — membership is a fact of construction, not upkeep.
            pub const ALL: [MappedCapability; mapped_capabilities!(@count $($variant)+)] =
                [$(MappedCapability::$variant),+];

            /// The registered capability id to invoke — `None` for the
            /// in-process catalog, which invokes nothing.
            pub fn capability_id(self) -> Option<&'static str> {
                match self {
                    $(MappedCapability::$variant => $capability_id,)+
                }
            }
        }
    };
    (@count $($variant:ident)+) => { 0usize $(+ { stringify!($variant); 1 })+ };
}

mapped_capabilities! {
    /// `k8s://.../<kind>/<name>` → `k8s.getManifest`.
    Manifest => Some("k8s.getManifest"),
    /// `k8s://.../<name>/events` → `k8s.listEvents`.
    Events => Some("k8s.listEvents"),
    /// `k8s://.../Pod/<name>/logs[/<container>]` → `k8s.podLogs`.
    Logs => Some("k8s.podLogs"),
    /// `k8s://contexts` → `k8s.listContexts`.
    Contexts => Some("k8s.listContexts"),
    /// `k8s://catalog`, assembled in-process from the registry and prompt
    /// library rather than by invoking any capability — which is exactly what
    /// `capability_id() == None` means. Replaces the old `"<catalog>"`
    /// sentinel string with something the type system understands.
    CatalogInProcess => None,
}
...
pub struct ResourceRead {
    pub capability: MappedCapability,
...
pub fn plan_read(uri: &ResourceUri, kinds: &dyn KindResolver) -> Result<ResourceRead, String> {
    ...
    let (context, namespace, kind, name, sub) = match uri {
        ResourceUri::Contexts => {
            return Ok(ResourceRead {
                capability: MappedCapability::Contexts,
                args: json!({}),
        ...
        ResourceUri::Catalog => {
            return Ok(ResourceRead {
                capability: MappedCapability::CatalogInProcess,
                args: json!({}),
                mime: "application/json",
            })
        }
    ...
    match sub {
        None => {
            ...
            Ok(ResourceRead { capability: MappedCapability::Manifest, args, mime: "application/yaml" })
        }
        Some(SubResource::Events) => Ok(ResourceRead {
            capability: MappedCapability::Events,
            args: json!({
                "context": context,
        ...
        Some(SubResource::Logs { container }) => {
            ...
            Ok(ResourceRead { capability: MappedCapability::Logs, args, mime: "text/plain" })
        }
    }
}
...
mod tests {
    ...
    fn a_manifest_read_maps_to_get_manifest_as_yaml() {
        ...
        assert_eq!(r.capability, MappedCapability::Manifest);
    ...
    fn an_events_read_filters_by_object() {
        ...
        let r = plan_read(&u, &StubKinds).unwrap();
        assert_eq!(r.capability, MappedCapability::Events);
    ...
    fn a_logs_read_maps_to_pod_logs_as_text() {
        ...
        let r = plan_read(&u, &StubKinds).unwrap();
        assert_eq!(r.capability, MappedCapability::Logs);
    ...
    fn the_fixed_resources_map_to_their_readers() {
        ...
        assert_eq!(c.capability, MappedCapability::Contexts);
        assert_eq!(c.capability.capability_id(), Some("k8s.listContexts"));
        let cat = plan_read(&ResourceUri::Catalog, &StubKinds).unwrap();
        assert_eq!(cat.capability, MappedCapability::CatalogInProcess);
        assert_eq!(cat.capability.capability_id(), None, "the catalog invokes nothing");
    }
    ...
    /// Every capability a resource read can invoke must be registered,
    /// read-only, and NOT consent-gated. Clients auto-fetch resources to
    /// populate context, so a resource read that raised a confirm dialog
    /// would be a consent-fatigue vector — the hazard #23's design warns
    /// about. Iterates the compiler-known `MappedCapability::ALL` (#196), so
    /// a new variant is guarded the moment it exists; the in-process catalog
    /// (`capability_id() == None`) invokes nothing and has nothing to check.
    #[test]
    fn every_mapped_capability_is_read_only_and_ungated() {
        ...
        for mapped in MappedCapability::ALL {
            let Some(id) = mapped.capability_id() else { continue };
            let cap = registry
                .get(id)
    ...
    /// `ALL` and the enum come from one macro list, so this can only fail if
    /// the macro itself regresses — kept as the cheap tripwire for that.
    #[test]
    fn all_has_no_duplicates() {
        let unique: std::collections::HashSet<MappedCapability> =
            MappedCapability::ALL.into_iter().collect();
        assert_eq!(unique.len(), MappedCapability::ALL.len());
    }
    ...
    /// Every capability that plan_read can name must appear in
    /// `MappedCapability::ALL`, and nothing in `ALL` may be unreachable from
    /// a URI. With the closed enum (#196) the first half is true by
    /// construction — `plan_read` can only name variants — so what this
    /// exercises is the second half: every variant is actually REACHABLE,
    /// i.e. `ALL` contains no dead entries. The exhaustive matches below
    /// still force a look here when a new ResourceUri or SubResource
    /// *variant* appears (fields are deliberately not guarded: `Logs { .. }`
    /// matches every field combination, so adding a field to an existing
    /// variant does not, and should not, break this).
    #[test]
    fn every_plan_read_branch_is_mapped_and_nothing_extra() {
        ...
        assert!(read.capability.capability_id().is_some(), "Contexts maps to a real capability");
        capabilities_named.insert(read.capability);

        // ResourceUri::Catalog — the in-process variant, a member of ALL
        // like any other now (no more sentinel string).
        let read = plan_read(&ResourceUri::Catalog, &AllowPodAndNode).unwrap();
        assert_eq!(read.capability, MappedCapability::CatalogInProcess);
        capabilities_named.insert(read.capability);
        ...
        // The set plan_read actually named must be exactly ALL — an ALL
        // variant no URI can reach is a dead entry the guards would be
        // "protecting" for nothing.
        let all_set: std::collections::HashSet<MappedCapability> =
            MappedCapability::ALL.into_iter().collect();
        assert_eq!(
            capabilities_named, all_set,
            "plan_read named capabilities differ from MappedCapability::ALL: \
             named={:?}, all={:?}",
            capabilities_named, all_set
        );
    }
