 ...
 *
 * `stableId` goes in each page's route; `scopeId` is the context key an app's
 * cluster scope names (#623). Pure, so the budget test (#581) times exactly
 * what the sidebar builds.
 */
export function appNavigation(plugins: InstalledExtension[], stableId: string, scopeId: string): ResourceNode | null {
  const apps = plugins.filter((p) => p.enabled && extensionEnabledFor(p, scopeId) && p.manifest.contributions.pages.length);
    ...
    children: apps.map((p) => ({
      ...
      children: p.manifest.contributions.pages.flatMap((page, index, pages) => {
        const leaf = (item: typeof page) => ({
          id: `route:${extensionClusterRoute(stableId, p.manifest.id, item.id)}`,
