...
- **extension budgets (release)** — the extension platform's performance budgets (#581) as a release build without coverage, uploaded as the `extension-budgets` artifact so they can be tracked across runs. The same tests run inside `frontend` and `backend`; there, as debug builds under coverage, they hold only counts and generous time ceilings. See [extensions/testing.md](extensions/testing.md#performance-budgets).
