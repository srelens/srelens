...
| APP-3 | Read Secrets or other core resources through the reader | I | `validate_app` (`crates/registry/src/extensions.rs`) requires a fixed, non-empty `group`, `version`, `plural` and `kind` of letters, digits, `.` and `-`, so the core group (Secrets, ConfigMaps, Pods) cannot be bound. The group must also be one a CustomResourceDefinition can declare: it needs a dot, so built-in groups such as `apps` and `batch` are refused, and `k8s.io` and its subdomains are refused as reserved (`group_problems` in `crates/registry/src/extensions/crd.rs`). The same rule runs when the inventory loads, so a stored app that breaks it is quarantined. Before dispatching, `extensions.read`, `extensions.resource` and `extensions.action` confirm that `{plural}.{group}` is a CustomResourceDefinition on the cluster (`require` in the same file, `custom_resource_definition_exists` in `crates/kube/src/crds.rs`), which also refuses a dotted group served by an aggregated API; a failed lookup refuses the call and says so. `k8s.listCustomResource` returns names, namespaces, ages and the declared printer columns (`list_custom_resource_capability` in `crates/kube/src/crds.rs`). | Shipped. Built-in groups refused in [#601]. See residual risk |
...
- **Any custom resource is readable in full.** Since [#601] a binding reaches only a
  CustomResourceDefinition, never a built-in or aggregated API, but it may name any CRD
...
| WEB-2 | Start a GitOps write with no consent prompt | E | `k8s.gitOpsAction` is also in `WEB_DENIED_CAPABILITIES`: the web host has no consent prompt, and no app scopes the call. | Shipped |
...
| [#555] | MCP-5: audit of calls from the UI |
