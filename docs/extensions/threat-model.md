...
| App secrets | `extension_secrets` in the desktop vault, `secrets.enc` under the MCP config dir, encrypted under the vault's one master key in the OS keychain (`apps/desktop/src-tauri/src/vault.rs`) | Credentials an app's `secret-reference` settings hold for other services (Datadog, GitHub, Grafana Cloud). |
...
| VULN-3 | An app's settings leak a credential (EXT-083) | I | Settings are typed by the manifest (#542), and a credential belongs in a `secret-reference` setting, whose value never enters the inventory. Every save is held to the declarations (`Manifest::check_setting_values` in `crates/plugin-host/src/manifest/settings.rs`, called from `checked_settings` in `crates/registry/src/extensions/app_settings.rs`), and any value for a `secret-reference`, even its reference, is refused. A `url` setting refuses a user name or password. The inventory writer refuses to save a secret setting that holds anything but its host-minted reference (`saved_form` in `crates/registry/src/extensions.rs`), whichever path put it there, and loading drops one that does. **Secret values ([#543]):** `extension.secretStore` keeps them in the desktop's existing vault, one more entry in `secrets.enc` under the one keychain-held master key, beside the MCP token (`VaultSecretStore` in `apps/desktop/src-tauri/src/extension_secrets.rs`); no per-secret keychain item and no second store. The value goes to the vault first and the reference is written after it (`change` in `crates/registry/src/extensions/secret_store.rs`). It needs the app's `extension.secretStore` grant, which a manifest must request exactly when it declares a secret setting (`Manifest::validate`) and which the install and update review shows with the settings it covers and the host's metadata. It fails closed: a set is refused unless the vault key is protected by the keychain, the biometric gate or the master password and the vault is unlocked, so the headless file-key fallback and a locked vault are refused with the reason, never written elsewhere. The value is write-only: the capability answers `{set}`; `extensions.list` returns the reference and reports a reference whose value is gone as not set; refusals never repeat the value (`secret` is refused by a deserializer that does not echo it); `SecretValue` and the vault's `Secrets` print no value in `Debug`; the MCP consent prompt blanks it before the window sees it (`shown_args` in `apps/desktop/src-tauri/src/mcp_confirm.rs`); the capability is `sensitive`, so its audit record blanks every argument value; and neither the settings export nor the settings bundle carries it (`secrets_for_export` in `apps/desktop/src-tauri/src/bundle_cmd.rs`). The store follows the inventory: after every inventory change the host deletes each stored secret no app references (`sweep`), so removing an app, resetting its settings, and an update or rollback that drops the setting or changes its type delete the secret. A setting reaches a capability only through a binding argument the capability marks settable, and never a secret (`PluginHost::interpolate`, `Capability::with_settable`); a secret reaches one only through `PluginHost::inject_secret`, into an argument the capability declares a secret slot, which none does yet. Other settings are still plain text in two places. The inventory stores them, and `extensions.list` returns them. An `extensions.configure` call is also recorded in the local audit log, from MCP or from Settings ? Apps (#555) (`audit.jsonl`, created with mode 0600 on Unix), whether consent is granted or denied. The capability is not sensitive-annotated, so `redact` (`crates/capability/src/audit.rs`) removes values by key name, which does not catch a setting named `credential` or `certificate`; the `settings` map is therefore redacted as a whole, keeping the action, the app ID and the setting names and blanking every value at any depth, and the recorded error is scrubbed of the same values, since a refused argument tends to be echoed by the refusal ([#605]). Other `extensions.configure` actions are recorded as before. Settings saved from Settings ? Apps go through the same capability and so through the same redaction: the app ID and the setting names are recorded, never a value. | Audit-log redaction of `settings` shipped ([#605]). Typed settings, with secret values refused from the inventory, shipped ([#542]). Keychain-backed secret storage, write-only, deleted with the app, shipped ([#543]). Injection into brokered HTTP headers planned in [#568]; web storage in [#522]. See residual risk |
...
- **An app secret is as safe as the vault, and no safer.** Anything running as the user
  while the vault is unlocked can read `secrets.enc` with the keychain's key, as it can
...
- **An MCP client can set or clear an app's secret with consent.** It cannot read one.
  Once a capability injects secrets (#568), setting one chooses which account the app
  talks to. The host's sentence for the prompt names only the action; the app and the
...
| ID | Threat | STRIDE | Mitigation | Status |
|---|---|---|---|---|
| WEB-1 | Read or change another user's apps, grants or settings | I, T | The inventory is one process-wide file, so the web host offers no apps. Web registries are built without a settings path, so `extensions.*` are never registered (`build_registry_with_paths` in `crates/registry/src/lib.rs`, used by `crates/server/src/bin/srelens-server.rs` and `--serve`), and every `extensions.*` ID, and `extension.secretStore`, is refused before dispatch anyway (`WEB_DENIED_CAPABILITIES` in `crates/server/src/api.rs`). `@srelens/core` also refuses to send a secret on the web (`setExtensionSecret`), so the value never leaves the page. | Shipped. Per-user apps planned in [#515] (epic [#522]) |
...
| [#568] | APP-2: brokered network access; VULN-3: the first secret slot, for HTTP headers |
