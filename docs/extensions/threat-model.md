...
| `extensions.action` | As `extensions.resource`, plus action, UID and resourceVersion | Yes | `resolve` in `crates/registry/src/extensions/resource.rs`, `execute` in `crates/kube/src/gitops.rs` |
...
`k8s.gitOpsAction` (`action_capability` in `crates/kube/src/gitops.rs`) is not an
`extensions.*` capability, but it is the write behind `extensions.action`, and any caller
of the registry can reach it directly.
...
| WEB-2 | Start a GitOps write with no consent prompt | E | `k8s.gitOpsAction` is also in `WEB_DENIED_CAPABILITIES`: the web host has no consent prompt, and no app scopes the call. The four action primitives ([#549]) are denied there for the same reason — what bounds one is an installed manifest fixing the kind and the template, which the web host has none of (`host_action_primitives_are_denied_on_web` in `crates/server/src/api.rs`). | Shipped |
...
| [#551] | APP-5: the built-in Flux and Argo CD action list moved into manifests, in place of `supported_actions` |
