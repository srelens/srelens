...
|---|---|---|
| `extensions.list` | Read-only | The installed apps, with revision, grants, settings and any quarantine reason. |
...
| `extensions.catalog` | Read-only | Browse the catalog, from a 24-hour cache. Reports the host's supported API versions as `hostApiVersions`. |
