...
- **#535:**
  ...
  - Those three send the request on under the checked context's pinned ID (its stable ID with the kubeconfig path made absolute), and a context lookup accepts either ID as well as a name, so a kubeconfig change mid-request cannot move it to another cluster. Stable IDs themselves are unchanged, since settings persist them.
